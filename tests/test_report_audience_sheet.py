...
def _sheet(tmp_path):
    ...
    return wb["Audience & leadership"], scope
...
def test_the_sheet_reports_executives_by_quarter_and_by_division(tmp_path):
    ...
    assert "GEB/GEB-1 INVOLVEMENT BY QUARTER" in labels
    assert "GEB/GEB-1 INVOLVEMENT BY DIVISION" in labels
...
# --- ACTIVITIES BY GEB/GEB-1 MEMBER ------------------------------------------------

def _geb_sheet(sources):
    """A sheet built from nothing but start dates and `bod_geb`."""
    frame = pd.DataFrame({
...
def _member_block(ws, title="ACTIVITIES BY GEB/GEB-1 MEMBER"):
...
def test_the_denominator_is_the_distinct_count_of_involved_activities():
    """Two members on one activity is still one activity with GEB/GEB-1."""
    ws = _geb_sheet(["A. Person; B. Person", "", ""])
    labels = _column_a(ws)
    denominator_row = labels.index("ACTIVITIES BY GEB/GEB-1 MEMBER") + 3

    assert ws.cell(row=denominator_row, column=1).value == "All activities with GEB/GEB-1"
...
def test_a_scope_without_any_named_member_says_so_instead_of_showing_nothing():
    ...
    assert "No GEB/GEB-1 member named on any in-scope activity" in _column_a(ws)

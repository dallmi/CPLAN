...
$manifest = @(
    ...
    @{ Path = "pipeline\scripts\process_cplan.py"; Marker = "excluded_total";                Why = "meta.json states how many rows were held back; the dashboard reads that file, and without the key it prints totals it cannot explain" },
    ...
    @{ Path = "pipeline\scripts\check_tracking_ids.py"; Marker = "def _read_xlsx_list";      Why = "reads the ID list from a workbook; without it an .xlsx list is read as text and every ID reports as missing" },
    @{ Path = "pipeline\scripts\check_tracking_ids.py"; Marker = "def write_xlsx";           Why = "the result workbook every run writes; an older copy names a path in the report and leaves no file there" },
    @{ Path = "pipeline\scripts\check_tracking_ids.py"; Marker = "DEFAULT_LIST_NAMES";       Why = "finds the ids.xlsx beside the launcher; without it a run with no -Ids fails on a missing argument instead of reading the list lying there" },
    @{ Path = ".gitignore";                    Marker = "ids.xlsx";                          Why = "keeps the tracking-ID list out of git - it names campaigns and carries notes, and an older copy leaves it sitting in git status waiting to be added" },
    @{ Path = "pipeline\scripts\check_tracking_ids.py"; Marker = "excluded";                Why = "a held-back activity answers 'excluded', not 'missing'; an older copy calls an activity that exists never-created, and somebody acts on that by creating it twice" },
    ...
    @{ Path = "trackids.ps1";                  Marker = '$Sheet';                            Why = "-Sheet and -Out reach the check; without them both switches are silently dropped and the run answers from the first sheet into the default file" },
    @{ Path = "trackids.ps1";                  Marker = 'if ($idsPath)';                     Why = "-Ids is optional, so a bare run reads the ids.xlsx beside the launcher; an older copy demands the switch and a double-click stops at a prompt" },
    ...
    @{ Path = "pipeline\dashboard\index.html";  Marker = "not for general circulation";     Why = "the dashboard says how many activities were held back; an older copy shows totals of a filtered set with nothing on the page admitting it" },
    @{ Path = "pipeline\mcp\server.py";        Marker = "not for general circulation";     Why = "tells the MCP client its database is already filtered; nothing the client can query reveals the gap, so an older copy answers counts as if they were exhaustive" },
    @{ Path = "pipeline\report\metrics.py";    Marker = "Hidden activities excluded";     Why = "the anomalies block states how many rows were held back; without it the Executive Summary and the agent pack report totals that are quietly smaller than the plan" },
    @{ Path = "pipeline\report\data.py";       Marker = "hidden_excluded";                  Why = "carries that count from the loader to every sheet; an older copy drops it and the figure reads zero on a run that excluded rows" },
    @{ Path = "pipeline\report\table_sheets.py"; Marker = "scope.hidden_excluded";          Why = "passes it into the Executive Summary's anomalies block" },
    @{ Path = "pipeline\report\agent_pack.py"; Marker = "scope.hidden_excluded";            Why = "passes it into the pack's anomalies block, which is the only place the agent can learn the data is incomplete" },
    @{ Path = "pipeline\report\agent_pack.py"; Marker = "def _held_back";                    Why = "the paragraph telling the agent the pack is not the whole plan; without it the agent answers totals as if they were exhaustive and has no way to know otherwise" },
    ...
    # Unlisted for a day, and the day cost a run: the pack column moved into a
    # module of its own, every report module imports it, and a machine without
    # it dies on `cannot import name packs` before the first CSV is read -
    # while this script reported every file it knew about as current.
    @{ Path = "pipeline\report\packs.py";      Marker = "PACK_LINK_COLUMN"; Why = "the module holding the measured pack-link column - a NEW file; without it report.cmd, agentpack.cmd and the daily refresh all fail to import, and the traceback names data.py rather than the file that is missing" },
    @{ Path = "pipeline\report\packs.py";      Marker = "MIN_LINK_RATE"; Why = "the floor under the link rate travels with the column - an older copy joins the pack list without ever saying the join went bad, and a badly joined 07-packs.csv looks exactly like a clean one" },
    # The pack column names were first taken from the form's documented field
    # labels rather than from an export. Three were wrong, and no test could
    # see it: the fixture was built from the same guess. A copy from before
    # this correction writes a 07-packs.csv whose Pack and Cluster columns are
    # empty in every row - the file that exists to name packs, naming none.
    ...
    # twice, and twice the estimate about these exports was wrong. This
    # measures it: cost per dimension in each file, beside coverage and
    ...
    # One board stopped being drawn. Its markup is frozen and a tested renderer
    # produces the page byte for byte, so what the agent supplies for it is a
    # JSON object rather than a picture. An old copy of any file below has no
    # contract at all: the agent meets a board it is told to draw, draws it,
    # and the four revision rounds that reached a conformant page are spent
    # again on every run.
    #
    # The grain is the part an old copy gets silently wrong rather than
    # loudly. 08-periods.csv carries every measure at year grain and the
    # activity count alone at quarter grain, so a contract citing quarter for
    # a total and year for a share sets two periods side by side and prints
    # neither.
    ...
    # renderer refused rather than fitting, and the agent -- given a refusal
    # and a reader waiting -- dropped four of seven teams to get an image out.
    #
    # The refusal was the worst of the three. Refusing does not stop a board
    # being produced; it moves the cutting to whoever holds the data, and that
    # cut leaves no mark on the finished picture. So the renderer fits now, and
    # where it cannot fit honestly it names what it left out.
    @{ Path = "pipeline\report\board_image.py"; Marker = "def fit_rows"; Why = "the panels fit whatever the period holds instead of refusing - an older copy hands a refusal to a caller who then trims the data silently, and the board says nothing about what left" },
    ...
    # notion of executive involvement at all. Four of the leadership board's
    # five panels plot one column, so the difference between "nobody is
    # involved" and "nobody fills the field" is most of that board.
    @{ Path = "pipeline\scripts\check_board_fill.py"; Marker = "COUNTS_ABSENCE"; Why = "the fill check that separates the four causes of a zero - a NEW file; without it a board panel plotting an unfilled column is indistinguishable from one plotting a real finding" },
    @{ Path = "boardfill.ps1"; Marker = "check_board_fill"; Why = "the launcher for that check - double-clickable via boardfill.cmd" },

    # First contact with a real pack: fourteen of twenty-three panels read
    ...
    # The whole three-run detour, in one line: the pack is read from wherever
    # the refresh copies it, and the folder a local build writes to keeps
    # whatever that build last left there. A leftover pack parses like any
    ...
    @{ Path = "pipeline\report\agent_builder.py"; Marker = "How an activity gets its pack"; Why = "the reading guide states the rule that picked each activity's pack and which of the counts to quote - an older copy leaves an agent to pick one of several columns that can differ by a factor of twenty, and every answer looks defensible" },

    # The chain, on trial: the pack field where it names a pack, the tracking
    # ID's segment where it does not. One column now carries values from two
    # places, so the source travels beside it - a merged column that cannot be
    # taken apart turns "this pack has 110 activities" and "someone planned
    # 110 activities into this pack" into the same sentence.
    @{ Path = "pipeline\report\packs.py"; Marker = "def resolve"; Why = "the pack an activity belongs to is resolved through the field first and the tracking ID second, and only ever to a pack the list answers to - an older copy has no such column, so everything downstream reads the field alone" },

...
**Registering a second project costs zero changes to `pipeline/portal`** — that is the whole point of the declaration/resolution split above, and it is asserted, not just claimed: `test_a_second_project_needs_no_portal_code` in `tests/test_portal_api.py` registers a real second project, group roles and manifest and only then calls the existing endpoint. Four steps:
...
3. **Drop a `resources.json`** beside the project's own documents, declaring its tiles as above.
4. **Write its manual, or omit that tile** — a manifest with no `manual` entry simply shows no manual tile; nothing else needs to exist for the rest of the page to work.
